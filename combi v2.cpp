...
inline int compare_result(char a, char b, int &in, int &out, int &score_up, int &word_slide, int &man_slide);
//void credit_window();
...
inline int compare_result(char a, char b, int &in, int &out, int &score_up, int &word_slide, int &man_slide)
{
	if (a==b)
	{
		...
		out++;
		score_up++;
		word_slide=word_slide-25;
		man_slide=man_slide+60;
	}
	else
	{
		in++;
...
Text current_letter("",font,20);
current_letter.setColor(Color::Green);
current_letter.setStyle(Text::Bold);
current_letter.setPosition(636,680);
...
input_string.setPosition(5,400);
	...
	if	(event.text.unicode==65)
	{
		...
		type_sound.play();
		input[for_in]='A';
		compare_result(input[for_in],output[for_out],for_in,for_out,score,wslide_position,man_position);
...
current_letter.setString(output[for_out]);
